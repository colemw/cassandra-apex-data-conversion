...
public class StudentByItemConvert implements Convert {
    ...
    public List<DataObject> getRowsByPartKey(String key, Session sourceSession) {
        ...
        for (Row row : results) {
            ...
            studenBytItems.add(new StudentByItemDAO(row));
        }
    ...
    public void setRowsByPartKey(Session targetSession, List<DataObject> inputList) {
        ...
        for (DataObject stbi : inputList) {
            ...
            StudentByItemDAO newStbi = convertRow(stbi);
    ...
    public StudentByItemDAO convertRow(DataObject stbi) {

        String cnttype = null;
        String cntid = null;
        Date duedate = null;

        StudentByItemDAO tempStbi = ((StudentByItemDAO) stbi);

        return new StudentByItemDAO(
                tempStbi.getCid()
                ,tempStbi.getCtype()
                ,tempStbi.getId()
                ,tempStbi.getDsc()
