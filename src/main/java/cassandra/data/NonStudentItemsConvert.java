...
public class NonStudentItemsConvert implements Convert {
    ...
    public List<DataObject> getRowsByPartKey(String key, Session sourceSession) {
        ...
        Map<UUID, NonStudentByItem> byItems = new HashMap<>();

        for (Row item : itemResultRaw) {
            NonStudentByItem newItem = new NonStudentByItem(item);
            byItems.put(newItem.getId(), newItem);
        }

        for (Row row : results) {
            NonStudentItems newItem = new NonStudentItems(row);
    ...
    public void setRowsByPartKey(Session targetSession, List<DataObject> inputList) {
        ...
        for (DataObject nsti : inputList) {
            ...
            NonStudentItems newNsti = convertRow(nsti);
    ...
    public NonStudentItems convertRow(DataObject nsti) {

        String cnttype = null;
        String cntid = null;
        Date duedate = null;

        NonStudentItems tempNsti = ((NonStudentItems) nsti);

        return new NonStudentItems(
                tempNsti.getCid()
                ,tempNsti.getCtype()
                ,tempNsti.getSo()
                ,tempNsti.getId()
